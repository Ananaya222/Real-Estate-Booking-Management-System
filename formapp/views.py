...
from formapp.forms import UserForm,UserProfileForm,UserUpdateForm,UserProfileFormUpdateForm,RealtorForm,RealtorProfileForm,EnquiryForm,RealtorUpdateForm
...
def enquiry(request):
    ...
    if request.method=='POST':
        ...
        if enquiry.is_valid():
            enquiry.save()
            submitted=True
            # messages.success(request,"Thank you for the enquiry, Our team will get back you soon .....")
    else:
        enquiry=EnquiryForm()
    return render(request,"enquiry.html",{'enquiry':enquiry,'submitted':submitted})

def search(request,area=None):
    all=allproperties.objects.all()
    area=request.GET.get('area')
    result=allproperties.objects.filter(area=area)
    return render(request,"search.html",{'result':result,'all':all})