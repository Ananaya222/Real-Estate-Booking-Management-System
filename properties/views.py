...
from properties.models import allproperties,PropertyBooking,Visit
from formapp.models import Realtors
from properties.forms import PropertiesForm,PropertiesEditForm
...
def property_detail(request,id=0):
    ...
    bathrooms = property.btallproperties.all()
    balconies = property.ballproperties.all()
...
def propertyEdit(request,id=0): 
    ...
    if request.method=='POST':
        form1=PropertiesEditForm(request.POST,request.FILES,instance=form1)
        if form1.is_valid():
            form1.save()
            return redirect('dashboard')
    else:
        form1=PropertiesEditForm(instance=form1)
